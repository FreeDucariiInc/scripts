...
local function on_setup_command(cmd)
    ...
    local local_player = entity_list.get_local_player()
    if local_player:get_prop("m_flDuckAmount")>0.85 then
...
local function on_antiaim(ctx)
    if not fake_duck_ref:get() or game_rules.get_prop("m_bIsValveDS")==0 then return end
    local local_player = entity_list.get_local_player()
    local m_flDuckAmount = local_player:get_prop("m_flDuckAmount")
    if m_flDuckAmount>0.6 and m_flDuckAmount<0.75 then
